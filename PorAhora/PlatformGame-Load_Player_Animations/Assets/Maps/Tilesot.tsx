<?xml version="1.0" encoding="UTF-8"?>
<tileset version="1.10" tiledversion="1.10.2" name="Tilesot" tilewidth="32" tileheight="32" tilecount="56" columns="8">
 <image source="Tileset.png" width="256" height="224"/>
 <wangsets>
  <wangset name="GrassFloor" type="corner" tile="-1">
   <wangcolor name="" color="#ff0000" tile="-1" probability="1"/>
   <wangtile tileid="0" wangid="0,0,0,1,0,0,0,0"/>
   <wangtile tileid="1" wangid="0,0,0,1,0,1,0,0"/>
   <wangtile tileid="2" wangid="0,0,0,0,0,1,0,0"/>
   <wangtile tileid="8" wangid="0,1,0,0,0,0,0,0"/>
   <wangtile tileid="9" wangid="0,1,0,0,0,0,0,1"/>
   <wangtile tileid="10" wangid="0,0,0,0,0,0,0,1"/>
  </wangset>
  <wangset name="GrassCeiling" type="corner" tile="-1">
   <wangcolor name="" color="#ff0000" tile="-1" probability="1"/>
   <wangtile tileid="8" wangid="0,0,0,1,0,0,0,0"/>
   <wangtile tileid="9" wangid="0,0,0,1,0,1,0,0"/>
   <wangtile tileid="10" wangid="0,0,0,0,0,1,0,0"/>
   <wangtile tileid="16" wangid="0,1,0,0,0,0,0,0"/>
   <wangtile tileid="17" wangid="0,1,0,0,0,0,0,1"/>
   <wangtile tileid="18" wangid="0,0,0,0,0,0,0,1"/>
  </wangset>
  <wangset name="WallL" type="corner" tile="-1">
   <wangcolor name="" color="#ff0000" tile="-1" probability="1"/>
   <wangtile tileid="4" wangid="0,0,0,1,0,0,0,0"/>
   <wangtile tileid="10" wangid="0,1,0,1,0,0,0,0"/>
   <wangtile tileid="12" wangid="0,1,0,0,0,0,0,0"/>
  </wangset>
  <wangset name="WallH" type="corner" tile="-1">
   <wangcolor name="" color="#ff0000" tile="-1" probability="1"/>
   <wangtile tileid="5" wangid="0,0,0,0,0,1,0,0"/>
   <wangtile tileid="13" wangid="0,0,0,0,0,0,0,1"/>
  </wangset>
  <wangset name="Platform" type="corner" tile="-1">
   <wangcolor name="" color="#ff0000" tile="-1" probability="1"/>
   <wangtile tileid="24" wangid="0,0,0,1,0,0,0,0"/>
   <wangtile tileid="25" wangid="0,0,0,1,0,1,0,0"/>
   <wangtile tileid="26" wangid="0,0,0,0,0,1,0,0"/>
  </wangset>
 </wangsets>
</tileset>
